...
class Interpreter(autonomic.UpdateChangeService):
            ...
            if self.config.has_option('Source Files','dictionary') :
                ...
                try :
                    ...
                    try: # Populate virtual and explicit entry lists
                        ...
                        logging.exception("Error: Something went wrong when trying to read the Dictionary Mapping File: ")
                        if hasattr(e, 'message'):
                            logging.exception(e.message)
                        else:
                            logging.exception(e)
    ...
    def writeVirtualEntryNano(self, nanopub) :
        for item in self.virtual_entry_list :
            ...
            nanopub.assertion.add((term,rdflib.RDFS.label,rdflib.Literal(str(item.Column[2:]), datatype=rdflib.XSD.string)))
            ...
            if (pd.notnull(item.Entity)) and (pd.isnull(item.Attribute)) :
                if ',' in item.Entity :
                    ...
                    for entity in entities :
                        nanopub.assertion.add((term,rdflib.RDFS.subClassOf,self.rdflibConverter(self.codeMapper(entity))))
                else :
                ...
                if ',' in item.Attribute :
                    ...
                    for attribute in attributes :
                        nanopub.assertion.add((term,rdflib.RDFS.subClassOf,self.rdflibConverter(self.codeMapper(attribute))))
                else :
            ...
            if (pd.notnull(item.inRelationTo)) :
                ...
                if (pd.notnull(item.Relation)) and (pd.isnull(item.Role)) :
                    nanopub.assertion.add((term,self.rdflibConverter(item.Relation),self.rdflibConverter(self.convertVirtualToKGEntry(item.inRelationTo))))
                ...
                # If there is a value in the Role column but not the Relation column ...
                    ...
                    role = rdflib.BNode()
                    nanopub.assertion.add((role,rdflib.RDF.type,self.rdflibConverter(self.convertVirtualToKGEntry(item.Role))))
                    nanopub.assertion.add((role,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(item.inRelationTo))))
                ...
                # If there is a value in the Role and Relation columns ...
                    ...
                    virtual_tuple["Relation"]=item.Relation
                    virtual_tuple["Role"]=item.Role
                    nanopub.assertion.add((term,sio.hasRole,self.rdflibConverter(self.convertVirtualToKGEntry(item.Role))))
                    nanopub.assertion.add((term,self.rdflibConverter(item.Relation),self.rdflibConverter(self.convertVirtualToKGEntry(item.inRelationTo))))
            nanopub.provenance.add((term,prov.generatedAtTime,rdflib.Literal("{:4d}-{:02d}-{:02d}".format(datetime.utcnow().year,datetime.utcnow().month,datetime.utcnow().day) + "T" + "{:02d}:{:02d}:{:02d}".format(datetime.utcnow().hour,datetime.utcnow().minute,datetime.utcnow().second) + "Z", datatype=rdflib.XSD.dateTime)))
            if pd.notnull(item.wasDerivedFrom) :
                if ',' in item.wasDerivedFrom :
                    ...
                    for derivedFromTerm in derivedFromTerms :
                        nanopub.provenance.add((term,prov.wasDerivedFrom, self.rdflibConverter(self.convertVirtualToKGEntry(derivedFromTerm))))
                else :
                    nanopub.provenance.add((term,prov.wasDerivedFrom, self.rdflibConverter(self.convertVirtualToKGEntry(item.wasDerivedFrom))))
            ...
            if pd.notnull(item.wasGeneratedBy) :
                if ',' in item.wasGeneratedBy :
                    ...
                    for generatedByTerm in generatedByTerms :
                        nanopub.provenance.add((term,prov.wasGeneratedBy, self.rdflibConverter(self.convertVirtualToKGEntry(generatedByTerm))))
                else :
                    nanopub.provenance.add((term,prov.wasGeneratedBy, self.rdflibConverter(self.convertVirtualToKGEntry(item.wasGeneratedBy))))
            ...
            for key in self.timeline_tuple :
                ...
                for timeEntry in timeline_tuple[key] :
                    ...
                    if 'Label' in timeEntry :
                        nanopub.assertion.add((tl_term,rdflib.RDFS.label,rdflib.Literal(str(timeEntry['Label']), datatype=rdflib.XSD.string)))
                    ...
                    if 'Unit' in timeEntry :
                        nanopub.assertion.add((tl_term,sio.hasUnit,self.rdflibConverter(self.codeMapper(timeEntry['Unit']))))
                    if 'inRelationTo' in timeEntry :
                        nanopub.assertion.add((tl_term,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(timeEntry['inRelationTo']))))
                nanopub.provenance.add((tl_term,prov.generatedAtTime,rdflib.Literal("{:4d}-{:02d}-{:02d}".format(datetime.utcnow().year,datetime.utcnow().month,datetime.utcnow().day) + "T" + "{:02d}:{:02d}:{:02d}".format(datetime.utcnow().hour,datetime.utcnow().minute,datetime.utcnow().second) + "Z", datatype=rdflib.XSD.dateTime)))
        ...
        try :
            if self.timeline_fn is not None :
                if v_column in self.timeline_tuple :
                    nanopub.assertion.add((term,rdflib.RDF.type,self.rdflibConverter(self.convertVirtualToKGEntry(v_column))))
                    ...
                    for timeEntry in self.timeline_tuple[v_column] :
                        ...
                        if 'Label' in timeEntry :
                            nanopub.assertion.add((term,rdflib.RDFS.label,rdflib.Literal(str(timeEntry['Label']), datatype=rdflib.XSD.string)))
                        if 'Start' in timeEntry and 'End' in timeEntry and timeEntry['Start'] == timeEntry['End']:
                        ...
                        if 'Unit' in timeEntry :
                            nanopub.assertion.add((term,sio.hasUnit,self.rdflibConverter(self.codeMapper(timeEntry['Unit']))))
                        if 'inRelationTo' in timeEntry :
                            nanopub.assertion.add((term,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(timeEntry['inRelationTo']))))
                    ...
                    if "Study" in v_tuple :
                        ...
                        if "Attribute" in v_tuple :
                            if ',' in v_tuple["Attribute"] :
                                ...
                                nanopub.assertion.add((term,rdflib.RDF.type,self.rdflibConverter(self.codeMapper(v_tuple["Attribute"]))))
                        if "Subject" in v_tuple :
                            nanopub.assertion.add((term,sio.hasIdentifier,rdflib.term.URIRef(self.prefixes[self.kb] + v_tuple["Subject"] + "-" + index)))
                        if "inRelationTo" in v_tuple :
                            if ("Role" in v_tuple) and ("Relation" not in v_tuple) :
                                role = rdflib.BNode()
                                nanopub.assertion.add((role,rdflib.RDF.type,self.rdflibConverter(self.convertVirtualToKGEntry(v_tuple["Role"],index))))
                                nanopub.assertion.add((role,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(v_tuple["inRelationTo"],index))))
                                nanopub.assertion.add((term,sio.hasRole,role))
                            elif ("Role" not in v_tuple) and ("Relation" in v_tuple) :
                                nanopub.assertion.add((term,self.rdflibConverter(v_tuple["Relation"]),self.rdflibConverter(self.convertVirtualToKGEntry(v_tuple["inRelationTo"],index))))
                            elif ("Role" not in v_tuple) and ("Relation" not in v_tuple) :
                                nanopub.assertion.add((term,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(v_tuple["inRelationTo"],index))))
                        nanopub.provenance.add((term,prov.generatedAtTime,rdflib.Literal("{:4d}-{:02d}-{:02d}".format(datetime.utcnow().year,datetime.utcnow().month,datetime.utcnow().day) + "T" + "{:02d}:{:02d}:{:02d}".format(datetime.utcnow().hour,datetime.utcnow().minute,datetime.utcnow().second) + "Z", datatype=rdflib.XSD.dateTime)))
                                ...
                                try :
                                    ...
                                    if "Entity" in a_tuple :
                                        if ',' in a_tuple["Entity"] :
                                            ...
                                            nanopub.assertion.add((term,rdflib.RDF.type,self.rdflibConverter(self.codeMapper(a_tuple["Entity"]))))
                                    if "isAttributeOf" in a_tuple :
                                        nanopub.assertion.add((term,sio.isAttributeOf,self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["isAttributeOf"],identifierString))))
                                    ...
                                    if "Unit" in a_tuple :
                                        nanopub.assertion.add((term,sio.hasUnit,self.rdflibConverter(self.codeMapper(a_tuple["Unit"]))))
                                    if "Time" in a_tuple :
                                        nanopub.assertion.add((term,sio.existsAt,self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["Time"], identifierString))))
                                        if self.checkVirtual(a_tuple["Time"]) :
                                            if a_tuple["Time"] not in vref_list :
                                                vref_list.append(a_tuple["Time"])
                                    if "Label" in a_tuple :
                                        nanopub.assertion.add((term,rdflib.RDFS.label,self.rdflibConverter(a_tuple["Label"])))
                                    if "Comment" in a_tuple :
                                        nanopub.assertion.add((term,rdflib.RDFS.comment,self.rdflibConverter(a_tuple["Comment"])))

                                    if "inRelationTo" in a_tuple :
                                        if ("Role" in a_tuple) and ("Relation" not in a_tuple) :
                                            role = rdflib.BNode()
                                            nanopub.assertion.add((role,rdflib.RDF.type,self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["Role"],identifierString))))
                                            nanopub.assertion.add((role,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["inRelationTo"],identifierString))))
                                            nanopub.assertion.add((term,sio.hasRole,role))
                                        elif ("Role" not in a_tuple) and ("Relation" in a_tuple) :
                                            nanopub.assertion.add((term,self.rdflibConverter(a_tuple["Relation"]),self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["inRelationTo"],identifierString))))
                                        elif ("Role" not in a_tuple) and ("Relation" not in a_tuple) :
                                            nanopub.assertion.add((term,sio.inRelationTo,self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["inRelationTo"],identifierString))))
                                except Exception as e:
                                    ...
                                    if row[col_headers.index(a_tuple["Column"])+1] != "" :
                                        ...
                                        try :
                                            if str(row[col_headers.index(a_tuple["Column"])+1]) == "nan" :
                                                ...
                                                nanopub.assertion.add((term,sio.hasValue,rdflib.Literal(str(row[col_headers.index(a_tuple["Column"])+1]), datatype=rdflib.XSD.integer)))
                                            elif self.isfloat(str(row[col_headers.index(a_tuple["Column"])+1])) :
                                                nanopub.assertion.add((term,sio.hasValue,rdflib.Literal(str(row[col_headers.index(a_tuple["Column"])+1]), datatype=rdflib.XSD.float)))
                                            else :
                                ...
                                try :
                                    ...
                                    if "wasDerivedFrom" in a_tuple :
                                        if ',' in a_tuple["wasDerivedFrom"] :
                                            ...
                                            for derivedFromTerm in derivedFromTerms :
                                                nanopub.provenance.add((term,prov.wasDerivedFrom, self.rdflibConverter(self.convertVirtualToKGEntry(derivedFromTerm,identifierString))))
                                                if self.checkVirtual(derivedFromTerm) :
                                                    if derivedFromTerm not in vref_list :
                                                        vref_list.append(derivedFromTerm)
                                        else :
                                            nanopub.provenance.add((term,prov.wasDerivedFrom, self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["wasDerivedFrom"],identifierString))))
                                    ...
                                    if "wasGeneratedBy" in a_tuple :
                                        if ',' in a_tuple["wasGeneratedBy"] :
                                            ...
                                            for generatedByTerm in generatedByTerms :
                                                nanopub.provenance.add((term,prov.wasGeneratedBy, self.rdflibConverter(self.convertVirtualToKGEntry(generatedByTerm,identifierString))))
                                                if self.checkVirtual(generatedByTerm) :
                                                    if generatedByTerm not in vref_list :
                                                        vref_list.append(generatedByTerm)
                                        else :
                                            nanopub.provenance.add((term,prov.wasGeneratedBy, self.rdflibConverter(self.convertVirtualToKGEntry(a_tuple["wasGeneratedBy"],identifierString))))
