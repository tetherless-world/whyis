...
class TestNeptuneGSPOperations:
    ...
    @patch('whyis.plugins.neptune.plugin.os.environ', {'AWS_ACCESS_KEY_ID': 'test_key', 'AWS_SECRET_ACCESS_KEY': 'test_secret'})
    @patch('whyis.plugins.neptune.plugin.ConjunctiveGraph')
    @patch('whyis.plugins.neptune.plugin.uuid.uuid4')
    def test_post_uses_temp_graph_by_default(self, mock_uuid, mock_conjunctive_graph):
        """Test that post uses temporary UUID graph by default."""
        from whyis.plugins.neptune.plugin import neptune_driver
        from rdflib import URIRef
        import whyis.plugins.neptune.plugin as plugin_module
        
        # Mock UUID generation
        test_uuid = 'test-uuid-5678'
        mock_uuid.return_value = test_uuid
        
        # Mock requests.Session at module level - need to mock it before creating driver
        with patch.object(plugin_module.requests, 'Session') as mock_session_class:
            mock_session_instance = Mock()
            mock_response = Mock()
            mock_response.ok = True
            mock_session_instance.post.return_value = mock_response
            ...
            # Mock ConjunctiveGraph.serialize
            mock_cg_instance = Mock()
            mock_cg_instance.serialize.return_value = b'<http://example.org/s> <http://example.org/p> <http://example.org/o> .'
            mock_conjunctive_graph.return_value = mock_cg_instance
            ...
            # Create a simple mock graph to post
            test_graph = Mock()
            test_graph.store = graph.store
            test_graph.identifier = URIRef('http://example.org/test-graph')
            
            # Call post
            graph.store.post(test_graph)
    ...
    @patch('whyis.plugins.neptune.plugin.ConjunctiveGraph')
    @patch('whyis.plugins.neptune.plugin.uuid.uuid4')
    def test_put_uses_temp_graph_by_default(self, mock_uuid, mock_conjunctive_graph):
        """Test that put uses temporary UUID graph by default."""
        from whyis.plugins.neptune.plugin import neptune_driver
        from rdflib import URIRef
        import whyis.plugins.neptune.plugin as plugin_module
        
        # Mock UUID generation
        test_uuid = 'test-uuid-9999'
        mock_uuid.return_value = test_uuid
        
        # Mock requests.Session at module level
        with patch.object(plugin_module.requests, 'Session') as mock_session_class:
            mock_session_instance = Mock()
            mock_response = Mock()
            mock_response.ok = True
            ...
            # Create a simple mock graph to put
            test_graph = Mock()
            test_graph.store = graph.store
            test_graph.identifier = URIRef('http://example.org/test-graph')
            
            # Call put
            graph.store.put(test_graph)
            
            # Verify PUT was called with temporary graph parameter
            assert mock_session_instance.put.called
            put_call_args = mock_session_instance.put.call_args
            assert put_call_args[1]['params']['graph'] == f'urn:uuid:{test_uuid}'
