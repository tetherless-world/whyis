...
:centimeter a :UnitOfMeasure;
  ...
  skos:definition "Some text to provide context.".
'''
...
class TestEntityResolverView(ApiTestCase):
    ...
    def test_context_lookup(self):
        ...
        self.app.db.add((Literal("centimeter"), bds.matchAllTerms, Literal("false")))
        self.app.db.add((Literal("centimeter"), bds.relevance, Literal(1.0)))
        self.app.db.add((Literal("Some text to provide context."), bds.search, Literal("context")))
        self.app.db.add((Literal("Some text to provide context."), bds.matchAllTerms, Literal("false")))
        self.app.db.add((Literal("Some text to provide context."), bds.relevance, Literal(1.0)))
        content = self.client.get("/?view=resolve&term=centimeter&context=context",
                                  headers={'Accept':'application/json'},
                                  follow_redirects=True)
        results = json.loads(content.data.decode('utf8'))
        self.assertEquals(len(results),1)
