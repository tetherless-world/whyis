...
docker run $WHYIS_IMAGE bash -c "curl -sL https://deb.nodesource.com/setup_12.x | bash - && apt-get install -y nodejs && mkdir -p test-results/js && cd tests/integration && npm install && npm run cypress:run"
