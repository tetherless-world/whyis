...
def get_nanopub_graph():
    ...
    fmt = sadi.mimeparse.best_match([mt for mt in list(current_app.dataFormats.keys()) if mt is not None],contentType)
    if fmt in current_app.dataFormats:
        inputGraph.parse(data=content, format=current_app.dataFormats[fmt])
    return inputGraph
